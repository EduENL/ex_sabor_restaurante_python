...
class RestauranteApp(tk.Tk):
    def __init__(self):
        ...
        self.search_button = ttk.Button(self.search_frame, text="Buscar", command=self.filtrar_restaurantes)
        self.search_button.pack(side=tk.LEFT, padx=5)

        self.search_entry = ttk.Entry(self.search_frame)
        self.search_entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        self.clear_button = ttk.Button(self.search_frame, text="Limpar", command=self.limpar_restaurantes)
    ...
    def atualizar_lista_restaurantes(self, filtro=None):
        """
        ...
        for restaurante in Restaurante.restaurantes:
            if filtro:
                if filtro.lower() not in restaurante._nome.lower() and filtro.lower() not in restaurante._categoria.lower():
                    continue
            self.tree.insert("", "end", values=(restaurante._nome, restaurante._categoria, restaurante.media_avaliacoes, restaurante.ativo))

    def filtrar_restaurantes(self):
        termo_pesquisa = self.search_entry.get()
        self.atualizar_lista_restaurantes(filtro=termo_pesquisa)

    def limpar_restaurantes(self):
