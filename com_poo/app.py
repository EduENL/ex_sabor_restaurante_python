...
def get_data_dir():
    if getattr(sys, 'frozen', False):
        # Se tiver executando como um executável
...
def carregar_dados():
    try:
        with open(ARQUIVO_DADOS, 'r', encoding='utf-8') as arquivo:
            ...
            Restaurante.restaurantes.clear() # Limpa a lista de restaurantes antes de carrefar os novos dados
...
def salvar_dados():
    ...
    with open(ARQUIVO_DADOS, 'w', encoding='utf-8') as arquivo:
        json.dump(dados, arquivo, indent=4, ensure_ascii=False) # Salva dados com indentação para melhor leitura

# Função principal do programa, que exibe o menu e executa as ações selenadas pelo usuário
def main():
    carregar_dados()

    while True:
        os.system('cls' if os.name == 'nt' else 'clear') # Limpar a tela de exibir menu
...
def cadastrar_restaurante():
    ...
    salvar_dados() # Salvar os dados após o cadastro
...
def habilitar_restaurante():
    nome = input("Digite o nome do restante que deseja habilitar/desabilitar: ")
    for restaurante in Restaurante.restaurantes:
...
# Função para adionar uma avaliação a um restaurante
            ...
            while True:
                try:
                    ...
                    if 0<= nota <= 10:
                        ...
                        salvar_dados() # Salvar os dados após a avaliação
                        return
                    else:
...
def alterar_restaurante():
    ...
    for restaurante in Restaurante.restaurantes:
        if restaurante._nome.lower() == nome.lower():
            novo_nome = input(f"Digite o nome do restaurante que seja alterar: ")
            ...
            salvar_dados() # Salvar dados após a alteração
            return
    print("Restaurante não encontrado")
        ...
        for restaurante in Restaurante.restaurantes:
            if restaurante._nome.lower() == nome.lower():
                ...
                if confirmacao.lower() == 's':
                    ...
                    print("Operação d eexclusão cancelada.")
                return
        print("Restaurante não encontrado.")
